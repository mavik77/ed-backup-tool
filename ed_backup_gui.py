...
# -----------------------------
ELITE_ORANGE = ("#ff7a00", "#ff7a00")
TRACK_DARK = ("#2a2a2a", "#2a2a2a")
ELITE_ORANGE_TEXT = "#ff7a00"
...
def is_windows_process_running(process_names: List[str]) -> Tuple[bool, List[str]]:
    ...
    try:
        ...
        matched = [name for name in process_names if name.lower() in out]
...
class EDBackupApp(ctk.CTk):
    def __init__(self):
        ...
        # Lock window size so banner doesn't shift
    ...
    def _app_dir(self) -> Path:
        ...
        - when frozen -> use folder next to the EXE
        - when running from source -> use folder next to the .py
        """
    ...
    def _set_window_icon(self):
        ...
        ico_path = app_dir / "ed_icon.ico"
        png_path = app_dir / "ed_icon.png"
    ...
    def _ui(self):
        ...
        banner_path = self._app_dir() / "ed_banner.png"
        if banner_path.exists():
            try:
                banner_img = Image.open(banner_path).convert("RGBA")
                self._banner_ctk = ctk.CTkImage(
                    light_image=banner_img, dark_image=banner_img, size=(940, 160)
                )
    ...
    def _fallback_header(self, header):
    ...
    def _poll_progress_queue(self):
        try:
            while True:
                ...
                if kind == "start":
                    ...
                    messagebox.showinfo("Backup complete", msg.get("summary", "Backup complete."))
        ...
        self.after(80, self._poll_progress_queue)
    ...
    def _worker_backup(self, tasks: List[Tuple[str, Path, str]], dest: Path, timestamp: str):
        try:
            ...
            total_units = max(1, total_files + len(existing))  # + manifest per zip
            done_units = 0
            ...
            lines = [f"✅ {zp.name}  (files: {n})" for zp, n in created]
            if skipped:
                lines.append("\nSkipped:")
